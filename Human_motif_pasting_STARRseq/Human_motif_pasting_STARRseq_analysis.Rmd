...
# all oligo library
Count_table_final <- read.delim("Oligo_counts.txt", stringsAsFactors = F)
...
df$log2FC_mut_swapped <- df$Activity-df$motif_mut_activity
formula_list <- list(lm(log2FC_mut_swapped ~ Wt_motif, df),
                     lm(log2FC_mut_swapped ~ New_motif, df),
                     lm(log2FC_mut_swapped ~ log2FC_wt_mut + Wt_motif, df),
                     lm(log2FC_mut_swapped ~ log2FC_wt_mut + New_motif, df),
                     lm(log2FC_mut_swapped ~ Wt_motif + New_motif, df),
                     lm(log2FC_mut_swapped ~ Wt_motif * New_motif, df),
...
motif_anno <- data.frame(row.names = names(sapply(List_motif_model_results, function(m) m$model_log2FC_mut_swapped_and_importance_no_number_plus_flanks$adj.r.squared)),
                         PCC = sapply(List_motif_model_results, function(m) m$model_log2FC_mut_swapped_and_importance_no_number_plus_flanks$PCC))
...
summary_lm <- do.call(rbind, lapply(names(List_motif_model_results), function(model){
  ...
  out <- as.data.frame(summary(List_motif_model_results[[model]]$model_log2FC_mut_swapped_and_importance_no_number_plus_flanks$Model_trained)$coefficients)
  out[,"Motif"] <- model
  out
}))
